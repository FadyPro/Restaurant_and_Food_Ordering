...
class CustomPageBuilderIndex extends Component
{
    public $model_id;
    public $paginate = 10;
    public $search = "";
    ...
    {
        $model = CustomPageBuilder::where('name','like','%'.$this->search.'%')
            ->paginate($this->paginate)->withQueryString();
        return view('livewire.admin.custom-page-builder.custom-page-builder-index', compact('model'));
    }
    ...
    {
        try{
            $item =  CustomPageBuilder::findOrFail($this->model_id);
            $item->delete();
    ...
    {
        try{
            $item =  CustomPageBuilder::findOrFail($id);
            $item->delete();
    ...
    public function updatedSelectAll($value)
    {
        if ($value)
        {
            $this->checked = CustomPageBuilder::pluck('id')->map(fn ($item) => (string) $item)->toArray();
        }else{
            $this->checked = [];
        }
//        $this->selectAll = true;
    ...
    {
        try{
            CustomPageBuilder::whereKey($this->checked)->delete();
            $this->checked = [];
            $this->selectAll = false;
