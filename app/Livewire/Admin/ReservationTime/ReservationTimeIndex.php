...
class ReservationTimeIndex extends Component
{
    public $model_id;
    public $paginate = 10;
    public $search = "";
    ...
    {
        $model = ReservationTime::query()
            ->paginate($this->paginate)->withQueryString();
        return view('livewire.admin.reservation-time.reservation-time-index',compact('model'));
    }
    ...
    {
        try{
            $item =  ReservationTime::findOrFail($this->model_id);
            $item->delete();
            ...
            return redirect()->to(route('admin.reservation-time.index'));
        }catch(\Exception $e){
    ...
    {
        try{
            $item =  ReservationTime::findOrFail($id);
            $item->delete();
    ...
    public function updatedSelectAll($value)
    {
        if ($value)
        {
            $this->checked = ReservationTime::pluck('id')->map(fn ($item) => (string) $item)->toArray();
        }else{
            $this->checked = [];
        }
//        $this->selectAll = true;
    ...
    {
        try{
            ReservationTime::whereKey($this->checked)->delete();
            $this->checked = [];
            $this->selectAll = false;
