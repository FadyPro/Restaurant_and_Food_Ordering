...
class CouponIndex extends Component
{
    ...
    {
        $model = Coupon::query()->where('name','like','%'.$this->search.'%')->paginate($this->paginate)->withQueryString();
    ...
    {
        try{
            $item =  Coupon::findOrFail($this->model_id);
            $item->delete();
    ...
    public function updatedSelectAll($value)
    {
        if ($value)
        {
            $this->checked = Coupon::pluck('id')->map(fn ($item) => (string) $item)->toArray();
        }else{
            $this->checked = [];
        }
//        $this->selectAll = true;
    ...
    {
        try{
            Coupon::whereKey($this->checked)->delete();
            $this->checked = [];
            $this->selectAll = false;
