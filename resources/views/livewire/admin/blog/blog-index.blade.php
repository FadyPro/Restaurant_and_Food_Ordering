            ...
            <div class="card card-primary">
                <div class="card-header">
                    ...
                    <div class="card-header-action" style="padding-left: 20px;" >
                        <a href="{{route('admin.blogs.create')}}" class="btn btn-primary" style="padding-left: 20px; text-align: right; float: right;">
                            New Item
                        </a>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped">
                            ...
                            <tr>
                                ...
                                <th wire:click="setSortBy('slug')" style="cursor: pointer">
                                    <a> Slug
                                        @if( $sortBy !== 'slug')
                                            <i class="fas fa-solid fa-arrow-up"></i>
                                        @elseif( $sortBy === 'slug' && $sortDirection === 'asc')
                                            <i class="fas fa-solid fa-arrow-down"></i>
                                        @else
                                            <i class="fas fa-solid fa-arrow-up"></i>
                                        @endif
                            ...
                            @foreach($model as $key=>$models)
                                <tr>
                                    ...
                                    <td> <img src="{{(!empty($models->image))? url('/uploads/blog/'.$models->image) : url('/uploads/no-image.png')}}"  width="60" height="60"> </td>
                                    <td>{{$models->category->name}}</td>
                                    <td>{{$models->user->name}}</td>
                                    <td style="width: 20%">{{$models->title}}</td>
                                    <td style="width: 20%">{{$models->slug}}</td>
                                    <td>
                                        @if($models->status === 1)
                                    ...
                                    <td>
                                        <a href="/admin/blogs/{{$models->id}}/edit"  class="btn btn-primary"><i class='fas fa-edit'></i></a>
