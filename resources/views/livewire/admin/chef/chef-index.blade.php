                        ...
                        <div class="accordion-body collapse" id="panel-body-1" data-parent="#accordion" style="">
                            <form wire:submit.prevent="updateTitles">
                                ...
                                <div class="form-group">
                                    ...
                                    <input wire:model="chef_top_title" type="text" class="form-control" name="chef_top_title"
                                           value="{{ @$titles['chef_top_title'] }}">
                                    @error('chef_top_title') <span class="error">{{ $message }}</span> @enderror
                                </div>
                                <div class="form-group">
                                    <label for="">Main Title</label>
                                    <input wire:model="chef_main_title" type="text" class="form-control" name="chef_main_title"
                                           value="{{ @$titles['chef_main_title'] }}">
                                    @error('chef_main_title') <span class="error">{{ $message }}</span> @enderror
                                </div>
                                <div class="form-group">
                                    <label for="">Sub Title</label>
                                    <input wire:model="chef_sub_title" type="text" class="form-control" name="chef_sub_title"
                                           value="{{ @$titles['chef_sub_title'] }}">
                                    @error('chef_sub_title') <span class="error">{{ $message }}</span> @enderror
                                </div>
            ...
            <div class="card card-primary">
                <div class="card-header">
                    ...
                    <div class="card-header-action" style="padding-left: 20px;" >
                        <a href="{{route('admin.chefs.create')}}" class="btn btn-primary" style="padding-left: 20px; text-align: right; float: right;">
                            New Item
                        </a>
                    </div>
                        ...
                        <table class="table table-striped">
                            ...
                            @foreach($model as $key=>$models)
                                <tr>
                                    ...
                                    <td> <img src="{{(!empty($models->image))? url('/uploads/chefs/'.$models->image) : url('/uploads/no-image.png')}}"  width="60" height="60"> </td>
                                    <td>{{$models->name}}</td>
                                    ...
                                    <td>
                                        <a href="/admin/chefs/{{$models->id}}/edit"  class="btn btn-primary"><i class='fas fa-edit'></i></a>
