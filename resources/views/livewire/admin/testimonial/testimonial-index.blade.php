                ...
                <div id="accordion">
                    <div class="accordion">
                        ...
                        <div class="accordion-body collapse" id="panel-body-1" data-parent="#accordion" style="">
                            <form wire:submit.prevent="updateTitles">
                                @csrf
                                <div class="form-group">
                                    <label for="">Top Title</label>
                                    <input wire:model="testimonial_top_title" type="text" class="form-control" name="testimonial_top_title"
                                           value="{{ @$titles['testimonial_top_title'] }}">
                                    @error('testimonial_top_title') <span class="error">{{ $message }}</span> @enderror
                                </div>
                                <div class="form-group">
                                    <label for="">Main Title</label>
                                    <input wire:model="testimonial_main_title" type="text" class="form-control" name="testimonial_main_title"
                                           value="{{ @$titles['testimonial_main_title'] }}">
                                    @error('testimonial_main_title') <span class="error">{{ $message }}</span> @enderror
                                </div>
                                <div class="form-group">
                                    <label for="">Sub Title</label>
                                    <input wire:model="testimonial_sub_title" type="text" class="form-control" name="testimonial_sub_title"
                                           value="{{ @$titles['testimonial_sub_title'] }}">
                                    @error('testimonial_sub_title') <span class="error">{{ $message }}</span> @enderror
                                </div>
        ...
        <div class="section-body">
            <div class="card card-primary">
                <div class="card-header">
                    <h4 style="width: 150px">Blogs List</h4>
                    <div>
                    ...
                    <div class="card-header-action" style="padding-left: 20px;" >
                        <a href="{{route('admin.testimonial.create')}}" class="btn btn-primary" style="padding-left: 20px; text-align: right; float: right;">
                            New Item
                        </a>
                    </div>
                        ...
                        <table class="table table-striped">
                            ...
                            @foreach($model as $key=>$models)
                                <tr>
                                    ...
                                    <td> <img src="{{(!empty($models->image))? url('/uploads/testimonial/'.$models->image) : url('/uploads/no-image.png')}}"  width="60" height="60"> </td>
                                    <td>{{$models->name}}</td>
                                    <td>{{$models->title}}</td>
                                    <td>{{$models->rating}}</td>
                                    <td style="width: 20%">{{$models->review}}</td>
                                    <td>
                                    ...
                                    <td>
                                        <a href="/admin/testimonial/{{$models->id}}/edit"  class="btn btn-primary"><i class='fas fa-edit'></i></a>
